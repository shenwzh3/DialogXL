...
class MaskedEdgeAttention(nn.Module):
    ...
    def forward(self, M, lengths, edge_ind):
        ...
        if attn_type == 'attn1':
            ...
            alpha = F.softmax(scale, dim=0).permute(1, 2, 0)  # (B, D, T)
            ...
            # print(mask_copy.size())
            # print(edge_ind_.shape)
            # exit()
            mask[edge_ind_] = 1
...
def batch_graphify(features, speaker_ids, speaker_vec, lengths, window_past, window_future, edge_type_mapping,
                   att_model, no_cuda):
    """
    ...
    features: (T, B, D)
    speaker_ids: (T, B) # the mask of speakers
p
    """

    edge_index, edge_norm, edge_type, node_features, speaker_features, packed_speaker_ids = [], [], [], [], [], []
    batch_size = features.size(1)  # (T, B, D)
    length_sum = 0
    edge_ind = []
    edge_index_lengths = []

    for j in range(batch_size):
        edge_ind.append(edge_perms(lengths[j], window_past, window_future))  # (B, num of edges)
    ...
    for j in range(batch_size):
        node_features.append(features[:lengths[j], j, :])  # (T,D)
        speaker_features.append(speaker_vec[:lengths[j], j, :])
        packed_speaker_ids.append(speaker_ids[:lengths[j], j])

        perms1 = edge_perms(lengths[j], window_past, window_future)  # (i,j) * num_of_edges
        perms2 = [(item[0] + length_sum, item[1] + length_sum) for item in
                  perms1]  # add a shift dut to combine all graph into on graph
        ...
        for item1, item2 in zip(perms1, perms2):
            ...
            speaker0 = speaker_ids[item1[0], j]
            speaker1 = speaker_ids[item1[1], j]

            if item1[0] < item1[1]:
                if speaker0 == speaker1:
                    edge_type.append(edge_type_mapping['10'])
                else:
                    edge_type.append(edge_type_mapping['00'])
                ...
                if speaker0 == speaker1:
                    edge_type.append(edge_type_mapping['11'])
                else:
                    edge_type.append(edge_type_mapping['01'])
    ...
    node_features = torch.cat(node_features, dim=0)  # (B*T, D)
    speaker_features = torch.cat(speaker_features, dim=0)  # (B*T, D)
    packed_speaker_ids = torch.cat(packed_speaker_ids, dim = 0) # (B*T)
    edge_index = torch.stack(edge_index).transpose(0, 1)  # (2, num_edges)
    edge_norm = torch.stack(edge_norm)  # (num_edges)
    edge_type = torch.tensor(edge_type)  # (num_edges)

    # if torch.cuda.is_available():
    if not no_cuda:
        node_features = node_features.cuda()
        speaker_features = speaker_features.cuda()
        packed_speaker_ids = packed_speaker_ids.cuda()
        edge_index = edge_index.cuda()
        edge_norm = edge_norm.cuda()
        edge_type = edge_type.cuda()
    return node_features, speaker_features, edge_index, edge_norm, edge_type, edge_index_lengths, packed_speaker_ids
...
class GatedSelection(nn.Module):
    def __init__(self, hidden_size):
        super(GatedSelection, self).__init__()
